#!/usr/bin/env bash
# launch_qwen3_4b 公共配置与串行运行逻辑（基于 Qwen3-4B）。
#
...
LAUNCH_4B_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="${PROJECT_DIR:-$(cd "${LAUNCH_4B_DIR}/.." && pwd)}"
export PROJECT_DIR

# ===== 基础模型 =====
export MODEL_PATH="${MODEL_PATH:-/cfs_turbo/jiaxicao/ckpt/hf/Qwen3-4B}"
