#!/usr/bin/env bash
# launch_qwen3_4b 总入口：串行执行四个数据集的全部训练。
#
...
#
# 用法：
#   bash launch_qwen3_4b/run_all.sh                 # 跑全部 4 个数据集
#   bash launch_qwen3_4b/run_all.sh apigen toolace  # 只跑指定数据集（按给定顺序）
#   SEED=2 bash launch_qwen3_4b/run_all.sh          # 环境变量会透传给各数据集脚本
#
# 说明：故意不开启 `set -e`，单个数据集脚本异常退出也会继续后续数据集，最后打印总汇总。
# 提示：整体耗时很长，建议放到后台运行，例如：
#   nohup bash launch_qwen3_4b/run_all.sh > run_all.log 2>&1 &
