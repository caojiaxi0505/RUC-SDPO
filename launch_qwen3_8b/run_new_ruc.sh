#!/usr/bin/env bash
# launch_qwen3_8b 新 RUC 方案入口：只执行新的 all-failed + Wilson gate + shrinkage RUC-SDPO-GRPO。
#
# 已完成的 grpo / sdpo / ruc-sdpo-grpo(sd0.01) / ruc-sdpo-grpo(sd0.02) 不会在这里重复执行。
#
# 用法：
#   bash launch_qwen3_8b/run_new_ruc.sh
#   bash launch_qwen3_8b/run_new_ruc.sh openr1_math medmcqa
#   UPLIFT_NUM_SAMPLES=2 bash launch_qwen3_8b/run_new_ruc.sh apigen
#
